...
import orm.conversation_orm as orm

COL = orm.ConversationOrm.COLLECTION_NAME
...
def test_find_by_id_valid(patch_mongo):
    ...
    oid = str(ObjectId())
    res = orm.ConversationOrm.find_by_id(oid)
...
def test_create_returns_string_id(patch_mongo):
    payload = {"title": "new"}
    new_id = orm.ConversationOrm.create(payload)
...
def test_update_valid_id_returns_modified_count(patch_mongo):
    oid = str(ObjectId())
    modified = orm.ConversationOrm.update(oid, {"$set": {"title": "x"}})
    assert isinstance(modified, int)
    assert modified == 1


def test_update_invalid_id_returns_zero(patch_mongo):
    modified = orm.ConversationOrm.update("bad-id", {"$set": {"title": "x"}})
    assert modified == 0


def test_delete_valid_id_returns_deleted_count(patch_mongo):
    oid = str(ObjectId())
    deleted = orm.ConversationOrm.delete(oid)
    assert deleted == 1


def test_delete_invalid_id_returns_zero(patch_mongo):
    deleted = orm.ConversationOrm.delete("bad-id")
    assert deleted == 0


def test_count_all_and_by_lang(patch_mongo):
    total = orm.ConversationOrm.count_all()
    by_lang = orm.ConversationOrm.count_by_lang("EN")
...
def test_search_by_field(patch_mongo):
    results = orm.ConversationOrm.search_by_field("title", "a", limit=5)
    assert isinstance(results, list)


def test_aggregate_by_lang(patch_mongo):
    agg = orm.ConversationOrm.aggregate_by_lang()
