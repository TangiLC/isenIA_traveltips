...
class PlugTypesETL:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if r.status_code == 200:
                    ...
                    # requests suit déjà les redirections; on laisse faire
                    pass
                elif r.status_code == 403 and attempt < max_retries:
                    # Ré-initialise les cookies en rechargeant la page, puis retente
    ...
    def download_images(self, df: pd.DataFrame):
        # Ne conserver que 3d-plug et 3d-sock; nommer <lettre>_plug.png et <lettre>_sock.png
        print(f"--- Téléchargement images: {len(df)} lignes ---")
    ...
    def run(self):
        ...
        print(f"ETL terminé: {len(df)} entrées")
        return df
